...
console.log('CORS configuration:', `https://${process.env.API_BASE_URL?.split('/')[2]}`); // Log the API base URL for debugging
server.register(fastifyCors, {
    origin: (origin, cb) => {
        const allowedOrigins = [
            'https://localhost', 
            process.env.DOMAIN_NAME ? `https://${process.env.DOMAIN_NAME}` : 'https://retrogamer',
            'https://127.0.0.1',
            `http://${process.env.DOMAIN_NAME}:3000`,
            'http://127.0.1:3000',
            `https://${process.env.API_BASE_URL?.split('/')[2]}`, // Extract domain from API_BASE_URL
        ]; // Add your frontend origin here dynamically
...
server.post('/api/heartbeat', async (request, reply) => {
    try {
