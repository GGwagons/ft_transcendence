...
class Society extends HTMLElement {
  ...
  private async fetchData(): Promise<void> {
    ...
    const onlineUsersResponse = await fetch(this.apiUrl, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
      },
    ...
    // Fetch friends
    this.apiUrl = getApiUrl('friends');
    const friendsResponse = await fetch(this.apiUrl, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('access_token')}`,
      },
      ...
      <table class="table-auto justify-cneter w-80% border-collapse border border-[#eebbc3] text-[#fffffe]">
        ...
        .map((friend) => {
          ...
          if (friend.avatarPath && friend.avatarPath !== '/public/df.jpeg') {
            const baseUrl = getApiUrlAvatar()
            fileUrl = `${baseUrl}${friend.avatarPath}`; // Construct the full URL for the avatar image
          }
  ...
  private setupListeners(): void {
      ...
      addFriendButtons.forEach((button) => {
        button.addEventListener('click', async (event) => {
          ...
          const isAlreadyFriend = this.friends.some(friend => friend.username === username);
          ...
          this.apiUrl = getApiUrl('send-friend-request');
