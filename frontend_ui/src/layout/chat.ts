import { getApiUrl, getApiUrlAvatar } from "../utils.js";
import { wssUrl } from "../utils.js";
...
class CustomChat extends HTMLElement {
  ...
  private async fetchCurrentUser(): Promise<{ username: string }> {
    this.apiUrl = getApiUrl("userInfo");
  ...
  > {
    this.apiUrl = getApiUrl("online-users");
    const response = await fetch(this.apiUrl, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("access_token")}`,
  ...
  private connectWebSocket(): void {
    ...
    this.socket = new WebSocket(
      `${wssUrl}ws?username=${this.currentUser}&token=${token}`
    );
    ...
    this.socket.onmessage = (event) => {
      ...
      // private message from the chat partner
        ...
        data.payload.from === this.chatPartner
      ) {
        this.chatPartner = data.payload.from;
      ...
      // invite message to play a game
        ...
        this.chatPartner = data.payload.from;
        this.showInvitationChoice(
          `You have been invited to play a game by ${data.payload.from}.`,
          data.payload.from
        );
      }
      // invitation accepted: redirect to the game page
      else if (
        data.type === "accept-invite" &&
        data.payload.from === this.chatPartner
      ) {
  ...
  private render(): void {
    this.shadowRoot!.innerHTML = `
      ...
      <div class="p-6 bg-[#232946] min-h-screen text-[#fffffe]">
        <h2 class="text-4xl font-bold mb-6 text-center text-[#ff8906]">CHAT</h2>
        <div class="flex flex-row justify-left items-center gap-4 py-4 pr-10">
          ...
          <a id="back" class="px-5 py-3 border-b-2 border-[#eebbc3] text-[#ff8906] font-bold rounded-xl hover:bg-[#ff6600]/10 hover:shadow-white hover:shadow-md transition-all ml-auto">Home</a>      
          </div>
            ...
            ${this.onlineUsers
              .map((user) => {
                ...
                if (user.profile && user.profile !== "/public/df.jpeg") {
                  const baseUrl = getApiUrlAvatar(); // Get the base URL for the avatar images
                  fileUrl = `${baseUrl}${user.profile}`; // Construct the full URL for the avatar image
                }
  ...
  private setupListeners(): void {
    ...
    const inviteButton = this.shadowRoot!.querySelector(
      "#invite-button"
    ) as HTMLButtonElement;
    const blockButton = this.shadowRoot!.querySelector(
      "#block-button"
    ) as HTMLButtonElement;
    const usernameInput = this.shadowRoot!.querySelector(
      "#username-input"
    ) as HTMLInputElement;
    const usernameInput2 = this.shadowRoot!.querySelector(
      "#username-input2"
    ) as HTMLInputElement;
    ...
    blockButton.addEventListener("click", async () => {
      ...
      try {
        ...
        this.apiUrl = getApiUrl(`get-email/${usernameToBlock}`);
        const emailResponse = await fetch(this.apiUrl, {
          method: "GET",
          headers: {
            Authorization: `Bearer ${localStorage.getItem("access_token")}`,
          },
        ...
        this.apiUrl = getApiUrl("block-user");
        const response = await fetch(this.apiUrl, {
          method: "POST",
          headers: {
  ...
  private displayMessage(message: string, sender: string): void {
    const chatBoard = this.shadowRoot!.querySelector(
      "#chat-board"
    ) as HTMLDivElement;
  ...
  private linkListeners(): void {
    // Look for the back button specifically since it's the only navigation we need
