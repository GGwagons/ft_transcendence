...
class GameRemote extends HTMLElement {
  ...
  private connectWebSocket(): void {
    ...
    this.socket = new WebSocket(`${wssUrl}ws-game?from=${this.currentUser}&to=${this.opponentUser}&token=${token}`);
  ...
  if (data.type === 'ball-update') {
    const { x, y, dx, dy, speed, radius } = data.payload;
    this.ball.x = x;
    this.ball.y = y;
    this.ball.dx = dx;
    this.ball.dy = dy;
    this.ball.speed = speed;
    this.ball.radius = radius; // Reset radius if needed
  }
        ...
        this.shadowRoot!.innerHTML = `<p class="text-red-500"> ${this.opponentUser} left, 5 seconds automatically goes back to chat</p>`;
        this.gameStarted = false;
        this.opponentReady = false;
        if (this.socket) {
          this.socket.close();
    ...
    this.socket.onclose = () => {
      ...
      this.apiUrl = getApiUrl('game-result');
      fetch(this.apiUrl, {
        method: 'POST',
        headers: {
